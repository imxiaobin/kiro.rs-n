...
interface CredentialCardProps {
  ...
  isInActiveRotationGroup?: boolean
}
...
function formatProxyDisplay(proxyPresetName?: string, proxyUrl?: string): string {
...
export function CredentialCard({
  ...
  const handleTestProxy = () => {
    testCredentialProxy.mutate(credential.id, {
      onSuccess: (result) => {
        const detail = `${result.message}，耗时 ${result.latencyMs}ms${
  ...
  return (
    <>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <span className={credential.activeRequests > 0 ? 'text-blue-500 font-medium' : ''}>
                {credential.activeRequests}
              </span>
            </div>
            <div>
            ...
            {credential.hasProxy && (
              <div className="col-span-2">
                ...
                <span className="font-medium">
                  {formatProxyDisplay(credential.proxyPresetName, credential.proxyUrl)}
                </span>
              </div>
            )}
          ...
          <div className="flex flex-wrap gap-2 pt-2 border-t">
            ...
            <Button
              ...
              title="检测当前凭据实际使用的代理链路"
            >
              {testCredentialProxy.isPending && (
      ...
      <CredentialProxyDialog
        ...
        credentialLabel={credential.email || `凭据 #${credential.id}`}
