...
/// - 其他 opus → claude-sonnet-4.5
/// - 所有 haiku → claude-haiku-4.5
pub fn map_model(model: &str) -> Option<String> {
    let model_lower = model.to_lowercase();

    if model_lower == "claude-opus-4-6" || model_lower == "claude-opus-4-6-thinking" {
...
mod tests {
    ...
    #[test]
    fn test_map_model_opus_4_6_exact() {
        let result = map_model("claude-opus-4-6");
        assert_eq!(result, Some("claude-sonnet-4.5".to_string()));
    }
