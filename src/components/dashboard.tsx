...
const LOAD_BALANCING_MODE_ORDER: LoadBalancingMode[] = [
  'priority',
  'balanced',
  'proxy_pair_rotation',
]
...
export function Dashboard({ onLogout }: DashboardProps) {
  ...
  const handleToggleLoadBalancing = () => {
    ...
    const currentIndex = LOAD_BALANCING_MODE_ORDER.indexOf(currentMode)
    const newMode = LOAD_BALANCING_MODE_ORDER[
      currentIndex >= 0 ? (currentIndex + 1) % LOAD_BALANCING_MODE_ORDER.length : 0
    ]

    setLoadBalancingMode({
      mode: newMode,
      proxyPairRotationIntervalMinutes: loadBalancingData?.proxyPairRotationIntervalMinutes,
      proxyPairRotationGroupSize: loadBalancingData?.proxyPairRotationGroupSize,
    }, {
      onSuccess: () => {
        const modeName = getLoadBalancingModeLabel(newMode)
            ...
            <div className="flex flex-wrap gap-2 xl:justify-end">
              ...
              {filteredCredentials.length > 0 && (
                <Button
                  ...
                  size="sm"
                  variant="outline"
                  disabled={queryingInfo || currentPageEnabledCount === 0}
