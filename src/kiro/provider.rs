...
impl KiroProvider {
    ...
    pub fn with_proxy(token_manager: Arc<MultiTokenManager>, proxy: Option<ProxyConfig>) -> Self {
        ...
        let initial_proxy = proxy.or_else(|| token_manager.global_proxy());
        // 预热：构建全局代理对应的 Client
        let initial_client = build_client(initial_proxy.as_ref(), 720, tls_backend)
            .expect("创建 HTTP 客户端失败");
        let mut cache = HashMap::new();
        cache.insert(initial_proxy, initial_client);
    ...
    fn client_for(&self, credentials: &KiroCredentials) -> anyhow::Result<Client> {
        let global_proxy = self.token_manager.global_proxy();
...
mod tests {
    ...
    fn test_build_headers() {
        ...
        let ctx = CallContext::new_for_test(1, credentials, "test_token".to_string());
