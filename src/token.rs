...
/// 全局配置存储
static COUNT_TOKENS_CONFIG: OnceLock<Mutex<CountTokensConfig>> = OnceLock::new();

/// 初始化 count_tokens 配置
///
/// 应在应用启动时调用一次
pub fn init_config(config: CountTokensConfig) {
    if let Some(existing) = COUNT_TOKENS_CONFIG.get() {
        *existing.lock() = config;
        return;
    }

    let _ = COUNT_TOKENS_CONFIG.set(Mutex::new(config));
}

/// 获取配置
fn get_config() -> Option<CountTokensConfig> {
    COUNT_TOKENS_CONFIG.get().map(|config| config.lock().clone())
}

/// 更新 count_tokens 远程调用使用的代理
pub fn update_proxy(proxy: Option<ProxyConfig>) {
    if let Some(config) = COUNT_TOKENS_CONFIG.get() {
        config.lock().proxy = proxy;
    }
}
